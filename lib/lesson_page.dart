...
class _LevelDetailsPageState extends State<LevelDetailsPage>
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: widget.level.lessons.length,
      vsync: this,
    ...
    _tabController.addListener(() {
      ...
      if (_tabController.index != _tabController.previousIndex) {
        final sectionTitle =
            widget.level.lessons[_tabController.index].sections[0].title;
        widget.updateProgress(
            widget.level.lessons[_tabController.index].title, sectionTitle, 1.0);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(widget.level.title,style: TextStyle(color: Colors.white),),
        leading: IconButton(
        ...
        bottom: TabBar(
          ...
          tabs: widget.level.lessons
              .map((lesson) => Tab(text: lesson.title))
              .toList(),
        ),
      ),
      body: TabBarView(
        controller: _tabController,
        children: widget.level.lessons.map((lesson) {
          return LessonCard(
            lesson: lesson,
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => LessonDetailsPage(
                ...
                updateProgress: widget.updateProgress, // Pass the function
              ),
            ),
          ).then((value) {
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => QuizPageInternal(
                              ...
                              onQuizCompleted: _onQuizCompleted, // Pass the function
                            ),
                          ),
                        ).then((value) {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Progress bar for the section (remove the progress bar since it's not needed)
          // LinearProgressIndicator(
          ...
          // Remove the Image.asset() line here
          // if (widget.section.imageUrl != null)
          // Image.asset(widget.section.imageUrl!),
          // SizedBox(height: 20),
          Text(
            section.content,
          ...
          if (section.quizQuestions.isNotEmpty)
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                  context,
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        VideoPage(videoUrl: section.videoUrl!),
                  ),
                ).then((value) {
                  updateProgress(1.0); // Update progress for the section
                });
              },
